import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@mariozechner/swarmz-coding-agent";
import { Container, Text } from "@mariozechner/swarmz-tui";
...
async function fetchGhMetadata(
	...
	try {
		const result = await swarmz.exec("gh", args);
...
export default function promptUrlWidgetExtension(swarmz: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = swarmz.getSessionName()?.trim();
		if (!currentName) {
			swarmz.setSessionName(desiredName);
			return;
		}
	...
	swarmz.on("before_agent_start", async (event, ctx) => {
		...
		void fetchGhMetadata(swarmz, match.kind, match.url).then((meta) => {
	...
	swarmz.on("session_switch", async (_event, ctx) => {
