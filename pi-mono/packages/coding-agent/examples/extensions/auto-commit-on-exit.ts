...
export default function (swarmz: ExtensionAPI) {
	swarmz.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await swarmz.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[swarmz] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await swarmz.exec("git", ["add", "-A"]);
		const { code: commitCode } = await swarmz.exec("git", ["commit", "-m", commitMessage]);
