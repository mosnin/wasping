...
import type { Component, EditorTheme, TUI } from "@midhaven/swarmz-tui";
...
export default function (swarmz: ExtensionAPI) {
	...
	swarmz.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await swarmz.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = swarmz.getThinkingLevel();
