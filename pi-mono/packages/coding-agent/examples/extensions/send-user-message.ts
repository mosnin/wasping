 ...
 *
 * Demonstrates swarmz.sendUserMessage() for sending user messages from extensions.
 * Unlike swarmz.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (swarmz: ExtensionAPI) {
	...
	swarmz.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			swarmz.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	swarmz.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				swarmz.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	swarmz.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				swarmz.sendUserMessage(args, { deliverAs: "followUp" });
	...
	swarmz.registerCommand("askwith", {
