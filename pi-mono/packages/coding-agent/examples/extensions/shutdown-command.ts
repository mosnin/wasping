...
export default function (swarmz: ExtensionAPI) {
	// Register a /quit command that cleanly exits swarmz
	swarmz.registerCommand("quit", {
		description: "Exit swarmz cleanly",
	...
	swarmz.registerTool({
		...
		description: "Complete a task and exit swarmz",
	...
	swarmz.registerTool({
		...
		description: "Deploy the application and exit swarmz",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await swarmz.exec("npm", ["run", "deploy", params.environment], { signal });
