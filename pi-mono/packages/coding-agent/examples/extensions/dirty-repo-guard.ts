...
async function checkDirtyRepo(
	...
	const { stdout, code } = await swarmz.exec("git", ["status", "--porcelain"]);
...
export default function (swarmz: ExtensionAPI) {
	swarmz.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(swarmz, ctx, action);
	});

	swarmz.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(swarmz, ctx, "fork");
	});
}
