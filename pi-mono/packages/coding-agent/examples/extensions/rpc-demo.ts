...
export default function (swarmz: ExtensionAPI) {
	...
	swarmz.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "swarmz RPC Demo (new session)" : "swarmz RPC Demo");
	...
	swarmz.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	swarmz.on("turn_end", async (_event, ctx) => {
	...
	swarmz.registerCommand("rpc-input", {
	...
	swarmz.registerCommand("rpc-editor", {
	...
	swarmz.registerCommand("rpc-prefill", {
