...
async function resolveGitHubRepo(swarmz: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await swarmz.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (swarmz: ExtensionAPI): void {
	swarmz.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(swarmz, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await swarmz.exec(
					"gh",
					[
						"issue",
						"list",
