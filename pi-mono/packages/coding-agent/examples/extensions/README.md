...
Example extensions for swarmz-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
swarmz --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.swarmz/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `swarmz.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs swarmz theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `swarmz.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using swarmz-ai's built-in Anthropic/OpenAI streaming via proxy |
