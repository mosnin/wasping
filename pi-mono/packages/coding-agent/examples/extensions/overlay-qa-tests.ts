 ...
 *
 * Usage: swarmz --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@mariozechner/swarmz-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@mariozechner/swarmz-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@mariozechner/swarmz-tui";
...
export default function (swarmz: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like swarmz-doom would need)
	swarmz.registerCommand("overlay-animation", {
	...
	swarmz.registerCommand("overlay-anchors", {
	...
	swarmz.registerCommand("overlay-margins", {
	...
	swarmz.registerCommand("overlay-stack", {
	...
	swarmz.registerCommand("overlay-overflow", {
	...
	swarmz.registerCommand("overlay-edge", {
	...
	swarmz.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	swarmz.registerCommand("overlay-maxheight", {
	...
	swarmz.registerCommand("overlay-sidepanel", {
	...
	swarmz.registerCommand("overlay-toggle", {
	...
	swarmz.registerCommand("overlay-passive", {
	...
	swarmz.registerCommand("overlay-focus", {
	...
	swarmz.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/swarmz/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/swarmz\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like swarmz-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (swarmz-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
