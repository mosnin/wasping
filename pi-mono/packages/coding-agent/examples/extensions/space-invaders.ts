...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@mariozechner/swarmz-tui";
...
export default function (swarmz: ExtensionAPI) {
	swarmz.registerCommand("invaders", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						swarmz.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
