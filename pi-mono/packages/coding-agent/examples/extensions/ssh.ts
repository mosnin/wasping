 ...
 *
 * Usage:
 *   swarmz -e ./ssh.ts --ssh user@host
 *   swarmz -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (swarmz: ExtensionAPI) {
	swarmz.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	swarmz.on("session_start", async (_event, ctx) => {
		...
		const arg = swarmz.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
