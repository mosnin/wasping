...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|swarmz] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Options:
  -l, --local    Install project-locally (.swarmz/settings.json)
...
Options:
  -l, --local    Remove from project settings (.swarmz/settings.json)
...
Update swarmz and installed packages.

Options:
  --self                  Update swarmz only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall swarmz even if the current version is latest

Short forms:
  ${APP_NAME} update                Update swarmz and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update swarmz             Update swarmz only (self works as alias to swarmz)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "swarmz";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand?: string[]): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of swarmz executable: ${entrypoint}`);
	}
}
