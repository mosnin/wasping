...
- Fixed `swarmz -p` treating prompts that start with YAML frontmatter as extension flags instead of user messages ([#4163](https://github.com/badlogic/pi-mono/issues/4163)).
...
- **Custom provider base URL overrides** - `swarmz.registerProvider()` now respects per-model `baseUrl` settings. See [docs/custom-provider.md](docs/custom-provider.md). ([#4063](https://github.com/badlogic/pi-mono/issues/4063)).
...
- **Self-update detection fix** - `swarmz` now correctly identifies and applies available updates. ([#3942](https://github.com/badlogic/pi-mono/issues/3942), [#3980](https://github.com/badlogic/pi-mono/issues/3980), [#3922](https://github.com/badlogic/pi-mono/issues/3922)).
...
- Added model-level `thinkingLevelMap` support in `models.json` and `swarmz.registerProvider()`, allowing models to expose only the thinking levels they actually support ([#3208](https://github.com/badlogic/pi-mono/issues/3208)).
- Added `shouldStopAfterTurn` agent loop callback for post-turn stop control, inherited from `@midhaven/swarmz-agent-core`. See [`packages/agent/README.md`](https://github.com/badlogic/pi-mono/blob/main/packages/agent/README.md).
...
- Fixed `swarmz.registerProvider()` to honor per-model `baseUrl` overrides ([#4063](https://github.com/badlogic/pi-mono/issues/4063)).
- Fixed self-update detection so `swarmz` correctly identifies when a newer version is available and applies updates ([#3942](https://github.com/badlogic/pi-mono/issues/3942), [#3980](https://github.com/badlogic/pi-mono/issues/3980), [#3922](https://github.com/badlogic/pi-mono/issues/3922)).
...
- Added Mistral Medium 3.5 built-in model support via `@midhaven/swarmz-ai` ([#4009](https://github.com/badlogic/pi-mono/pull/4009) by [@technocidal](https://github.com/technocidal)).
...
- Added top-level `name` support to `swarmz.registerProvider()` so extension-registered providers can show a friendly name in `/login` ([#3956](https://github.com/badlogic/pi-mono/issues/3956)).
...
- Fixed the coding-agent README See Also link to point at `@midhaven/swarmz-agent-core` ([#4023](https://github.com/badlogic/pi-mono/issues/4023)).
...
- Swarmz update checks now use `swarmz.dev` and identify Swarmz with a `swarmz/<version>` user agent. See [docs/packages.md](docs/packages.md). ([#3877](https://github.com/badlogic/pi-mono/pull/3877) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Changed Swarmz version checks to identify Swarmz with a `swarmz/<version>` user agent ([#3877](https://github.com/badlogic/pi-mono/pull/3877) by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed `swarmz update` to skip self-update reinstalls when the installed version is already current ([#3853](https://github.com/badlogic/pi-mono/issues/3853)).
- Fixed Cloudflare Workers AI attribution headers to honor the install telemetry setting.
- Fixed `swarmz update --self` detection and execution for Windows package-manager shim installs, including symlinked global package roots, and print the manual fallback command when self-update fails ([#3857](https://github.com/badlogic/pi-mono/issues/3857)).
...
- `swarmz update` can now update swarmz itself in addition to installed swarmz packages. See [docs/packages.md](docs/packages.md). ([#3680](https://github.com/badlogic/pi-mono/pull/3680) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Added `swarmz update` support for updating swarmz itself in addition to installed swarmz packages ([#3680](https://github.com/badlogic/pi-mono/pull/3680) by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed extension `swarmz.setSessionName()` updates to refresh the interactive terminal title immediately ([#3686](https://github.com/badlogic/pi-mono/issues/3686))
...
- Fixed extension flag docs to show `swarmz.getFlag()` using registered flag names without the CLI `--` prefix ([#3614](https://github.com/badlogic/pi-mono/issues/3614))
...
- Fixed Windows git package installs to bypass `cmd.exe` for native git commands, so install paths containing spaces no longer break `swarmz install git:...` with `fatal: Too many arguments` ([#3642](https://github.com/badlogic/pi-mono/issues/3642))
...
- Fixed `swarmz-coding-agent` shipping `uuid@11`, which triggered `npm audit` moderate vulnerability reports for downstream installs; the package now depends on `uuid@14` ([#3577](https://github.com/badlogic/pi-mono/issues/3577))
...
- Added `PI_OAUTH_CALLBACK_HOST` support for built-in OAuth login flows, allowing local callback servers used by `swarmz auth` to bind to a custom interface instead of hardcoded `127.0.0.1` ([#3409](https://github.com/badlogic/pi-mono/pull/3409) by [@Michaelliv](https://github.com/Michaelliv))
...
- Changed `swarmz update` to batch npm package updates per scope and run git package updates with bounded parallelism, reducing multi-package update time while preserving skip behavior for pinned and already-current packages ([#2980](https://github.com/badlogic/pi-mono/issues/2980))
...
- Fixed `swarmz update` reinstalling npm packages that are already at the latest published version by checking the installed package version before running `npm install <pkg>@latest` ([#3000](https://github.com/badlogic/pi-mono/issues/3000))
...
- Fixed Windows pnpm global install detection to recognize `\\.pnpm\\` store paths, so update notices now suggest `pnpm install -g @midhaven/swarmz-coding-agent` instead of falling back to npm ([#3378](https://github.com/badlogic/pi-mono/issues/3378))
- Fixed missing `@sinclair/typebox` runtime dependency in `@midhaven/swarmz-coding-agent`, so strict pnpm installs no longer fail with `ERR_MODULE_NOT_FOUND` when starting `swarmz` ([#3434](https://github.com/badlogic/pi-mono/issues/3434))
- Fixed xterm uppercase typing in the interactive editor by decoding printable `modifyOtherKeys` input and normalizing shifted letter matching, so `Shift+letter` no longer disappears in `swarmz` ([#3436](https://github.com/badlogic/pi-mono/issues/3436))
...
- Fixed skill resolution to dedupe symlinked aliases by canonical path, so `swarmz config` no longer shows duplicate skill entries when `~/.swarmz/agent/skills` points to `~/.agents/skills` ([#3417](https://github.com/badlogic/pi-mono/pull/3417) by [@rwachtler](https://github.com/rwachtler))
- Fixed OpenRouter request attribution to include Swarmz app headers (`HTTP-Referer: https://swarmz.dev`, `X-OpenRouter-Title: swarmz`, `X-OpenRouter-Categories: cli-agent`) when sessions are created through the coding-agent SDK and install telemetry is enabled ([#3414](https://github.com/badlogic/pi-mono/issues/3414))
...
- Fixed missing root exports for `RpcClient` and RPC protocol types from `@midhaven/swarmz-coding-agent`, so ESM consumers can import them from the main package entrypoint ([#3275](https://github.com/badlogic/pi-mono/issues/3275))
...
- Fixed Gemma 4 thinking level mapping to route between `MINIMAL` and `HIGH`, and map Swarmz reasoning levels to the model's supported thinking levels ([#2903](https://github.com/badlogic/pi-mono/pull/2903) by [@aadishv](https://github.com/aadishv))
...
- Fixed OpenAI-compatible completions streaming usage accounting to preserve `prompt_tokens_details.cache_write_tokens` and normalize OpenRouter `cached_tokens`, preventing incorrect cache read/write token and cost reporting in swarmz ([#2802](https://github.com/badlogic/pi-mono/issues/2802))
...
import {
  ...
  getAgentDir,
  SessionManager,
} from "@midhaven/swarmz-coding-agent";
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@midhaven/swarmz-ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Fixed skill discovery to stop recursing once a directory contains `SKILL.md`, and to ignore root `*.md` files in `.agents/skills` while keeping root markdown skill files supported in `~/.swarmz/agent/skills`, `.swarmz/skills`, and package `skills/` directories ([#2603](https://github.com/badlogic/pi-mono/issues/2603))
...
- Added a startup onboarding hint in the interactive header telling users swarmz can explain its own features and documentation ([#2620](https://github.com/badlogic/pi-mono/pull/2620) by [@ferologics](https://github.com/ferologics))
...
- Added support for `PI_TUI_WRITE_LOG` directory paths, creating a unique log file (`tui-<timestamp>-<pid>.log`) per instance for easier debugging of multiple swarmz sessions ([#2508](https://github.com/badlogic/pi-mono/pull/2508) by [@mrexodia](https://github.com/mrexodia))
...
- Fixed `models.json` shell-command auth and headers to resolve at request time instead of being cached into long-lived model state. swarmz now leaves TTL, caching, and recovery policy to user-provided wrapper commands because arbitrary shell commands need provider-specific strategies ([#1835](https://github.com/badlogic/pi-mono/issues/1835))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@midhaven/swarmz-ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Fixed `swarmz update` for git packages to fetch only the tracked target branch with `--no-tags`, reducing unrelated branch and tag noise while preserving force-push-safe updates ([#2548](https://github.com/badlogic/pi-mono/issues/2548))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@midhaven/swarmz-coding-agent`.
...
- Added structured `sourceInfo` to `swarmz.getAllTools()` results for built-in, SDK, and extension tools ([#1734](https://github.com/badlogic/pi-mono/issues/1734))
...
- Fixed `swarmz update` for git packages to skip destructive reset, clean, and reinstall steps when the fetched target already matches the local checkout ([#2503](https://github.com/badlogic/pi-mono/issues/2503))
...
- Added `ToolCallEventResult` to the `@midhaven/swarmz-coding-agent` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Fixed project-local npm package updates to install npm `latest` instead of reusing stale saved dependency ranges, and added `Did you mean ...?` suggestions when `swarmz update <source>` omits the configured npm or git source prefix ([#2459](https://github.com/badlogic/pi-mono/issues/2459))
...
- Changed package startup behavior so installed unpinned packages are no longer checked or updated during startup. Use `swarmz update` to apply npm/git package updates, while interactive mode now checks for available package updates in the background and notifies you when updates are available ([#1963](https://github.com/badlogic/pi-mono/issues/1963))
...
- Added `createLocalBashOperations()` export so extensions and SDK callers can wrap swarmz's built-in local bash backend for `user_bash` handling and other custom bash integrations ([#2299](https://github.com/badlogic/pi-mono/issues/2299))
...
- Faster startup by lazy-loading `@midhaven/swarmz-ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@midhaven/swarmz-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed explicit `swarmz -e <path>` extensions losing command and tool conflicts to discovered extensions by giving CLI-loaded extensions higher precedence ([#1896](https://github.com/badlogic/pi-mono/issues/1896))
...
- Fixed `swarmz config` misclassifying `~/.agents/skills` as project-scoped in non-git directories under `$HOME`, so toggling those skills no longer writes project overrides to `.swarmz/settings.json` ([#1915](https://github.com/badlogic/pi-mono/issues/1915))
...
- Fixed parallel swarmz processes failing with false "No API key found" errors due to immediate lockfile contention on `auth.json` and `settings.json` ([#1871](https://github.com/badlogic/pi-mono/issues/1871))
...
- Fixed `@midhaven/swarmz-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@midhaven/swarmz-ai` entry. OAuth login and refresh must be imported from `@midhaven/swarmz-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@midhaven/swarmz-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@midhaven/swarmz-ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
## [0.55.4] - 2026-03-02

### New Features

- Runtime tool registration now applies immediately in active sessions. Tools registered via `swarmz.registerTool()` after startup are available to `swarmz.getAllTools()` and the LLM without `/reload` ([docs/extensions.md](docs/extensions.md), [examples/extensions/dynamic-tools.ts](examples/extensions/dynamic-tools.ts), [#1720](https://github.com/badlogic/pi-mono/issues/1720)).
...
- Fixed `swarmz.registerTool()` dynamic registration after session initialization. Tools registered in `session_start` and later handlers now refresh immediately, become active, and are visible to the LLM without `/reload` ([#1720](https://github.com/badlogic/pi-mono/issues/1720))
...
- Extensions can dynamically remove custom providers via `swarmz.unregisterProvider(name)`, restoring any built-in models that were overridden, without requiring `/reload` ([docs](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/custom-provider.md)).
...
- `swarmz.unregisterProvider(name)` removes a dynamically registered provider and its models from the registry without requiring `/reload`. Built-in models that were overridden by the provider are restored ([#1669](https://github.com/badlogic/pi-mono/pull/1669) by [@aliou](https://github.com/aliou)).
...
- Fixed SIGINT being delivered to swarmz while the process is suspended (e.g. via `ctrl+z`), which could corrupt terminal state on resume ([#1668](https://github.com/badlogic/pi-mono/pull/1668) by [@aliou](https://github.com/aliou)).
...
- `swarmz.getAllTools()` now exposes tool parameters in addition to name and description, enabling richer extension integrations ([#1416](https://github.com/badlogic/pi-mono/pull/1416) by [@marchellodev](https://github.com/marchellodev)).
...
- `swarmz.getAllTools()` now returns tool parameters in addition to name and description ([#1416](https://github.com/badlogic/pi-mono/pull/1416) by [@marchellodev](https://github.com/marchellodev))
...
- `swarmz <package> --help` and invalid subcommands now show helpful output instead of failing silently ([#1347](https://github.com/badlogic/pi-mono/pull/1347) by [@ferologics](https://github.com/ferologics))
...
- Fixed extensions setting not respecting `package.json` `swarmz.extensions` manifest when directory is specified directly ([#1302](https://github.com/badlogic/pi-mono/pull/1302) by [@hjanuschka](https://github.com/hjanuschka))
...
- Added local path support for `swarmz install` and `swarmz remove` with relative paths stored against the target settings file ([#1216](https://github.com/badlogic/pi-mono/issues/1216))
...
- **Bash spawn hook**: Extensions can now intercept and modify bash commands before execution via `swarmz.setBashSpawnHook()`. Adjust the command string, working directory, or environment variables. See [docs/extensions.md](docs/extensions.md). ([#1160](https://github.com/badlogic/pi-mono/pull/1160) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Fixed `swarmz update` not updating npm/git packages when called without arguments ([#1151](https://github.com/badlogic/pi-mono/issues/1151))
...
- Fixed `swarmz update <source>` installing packages locally when the source is only registered globally ([#1163](https://github.com/badlogic/pi-mono/pull/1163) by [@aliou](https://github.com/aliou))
...
- **OSC 52 clipboard support for SSH/mosh** - The `/copy` command now works over remote connections using the OSC 52 terminal escape sequence. No more clipboard frustration when using swarmz over SSH. ([#1069](https://github.com/badlogic/pi-mono/issues/1069) by [@gturkoglu](https://github.com/gturkoglu))
...
- Removed `discoverAuthStorage` and `discoverModels` from the SDK. `AuthStorage` and `ModelRegistry` now default to `~/.swarmz/agent` paths unless you pass an `agentDir` ([#645](https://github.com/badlogic/pi-mono/issues/645))
...
- Extension package management with `swarmz install`, `swarmz remove`, `swarmz update`, and `swarmz list` commands ([#645](https://github.com/badlogic/pi-mono/issues/645))
...
- Glob pattern support with minimatch in package filters, top-level settings arrays, and swarmz manifest (e.g., `"!funky.json"`, `"*.ts"`) ([#645](https://github.com/badlogic/pi-mono/issues/645))
- `/reload` command to reload extensions, skills, prompts, and themes ([#645](https://github.com/badlogic/pi-mono/issues/645))
- `swarmz config` command with TUI to enable/disable package and top-level resources via patterns ([#938](https://github.com/badlogic/pi-mono/issues/938))
...
- Fixed error messages showing hardcoded `~/.swarmz/agent/` paths instead of respecting `PI_CODING_AGENT_DIR` ([#887](https://github.com/badlogic/pi-mono/pull/887) by [@aliou](https://github.com/aliou))
...
- Added widget placement option for extension widgets via `widgetPlacement` in `swarmz.addWidget()` ([#850](https://github.com/badlogic/pi-mono/pull/850) by [@marckrenn](https://github.com/marckrenn))
...
- `swarmz.setLabel(entryId, label)` in ExtensionAPI for setting per-entry labels from extensions ([#806](https://github.com/badlogic/pi-mono/issues/806))
...
- Extension commands can provide argument auto-completions via `getArgumentCompletions` in `swarmz.registerCommand()` ([#775](https://github.com/badlogic/pi-mono/pull/775) by [@ribelo](https://github.com/ribelo))
...
- Fixed extension messages rendering twice on startup when `swarmz.sendMessage({ display: true })` is called during `session_start` ([#765](https://github.com/badlogic/pi-mono/pull/765) by [@dannote](https://github.com/dannote))
...
- Piped stdin now works correctly: `echo foo | swarmz` is equivalent to `swarmz -p foo`. When stdin is piped, print mode is automatically enabled since interactive mode requires a TTY ([#708](https://github.com/badlogic/pi-mono/issues/708))
...
- CLI process title is now set to `swarmz` for easier process identification ([#742](https://github.com/badlogic/pi-mono/pull/742) by [@richardgill](https://github.com/richardgill))
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@midhaven/swarmz-tui`, `@midhaven/swarmz-ai`, `@midhaven/swarmz-coding-agent`) are accessible to extensions without filesystem node_modules.
...
- `swarmz.getAllTools()` now returns `ToolInfo[]` (with `name` and `description`) instead of `string[]`. Extensions that only need names can use `.map(t => t.name)`. ([#648](https://github.com/badlogic/pi-mono/pull/648) by [@carsonfarmer](https://github.com/carsonfarmer))

### Added

- Session naming: `/name <name>` command sets a display name shown in the session selector instead of the first message. Useful for distinguishing forked sessions. Extensions can use `swarmz.setSessionName()` and `swarmz.getSessionName()`. ([#650](https://github.com/badlogic/pi-mono/pull/650) by [@scutifer](https://github.com/scutifer))
...
- External edits to `settings.json` while swarmz is running are now preserved when swarmz saves settings ([#527](https://github.com/badlogic/pi-mono/pull/527) by [@ferologics](https://github.com/ferologics))
...
- Session picker (`swarmz -r`) and `--session` flag now support searching/resuming by session ID (UUID prefix) ([#495](https://github.com/badlogic/pi-mono/issues/495) by [@arunsathiya](https://github.com/arunsathiya))
...
- Extensions can now send user messages via `swarmz.sendUserMessage()` ([#483](https://github.com/badlogic/pi-mono/issues/483))
...
- Managed binaries (`fd`, `rg`) now stored in `~/.swarmz/agent/bin/` instead of `tools/`, eliminating false deprecation warnings ([#470](https://github.com/badlogic/pi-mono/pull/470) by [@mcinteerj](https://github.com/mcinteerj))
...
- OAuth refresh no longer logs users out when multiple swarmz instances are running ([#466](https://github.com/badlogic/pi-mono/pull/466) by [@Cursivez](https://github.com/Cursivez))
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@midhaven/swarmz-coding-agent";
export default function (swarmz: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@midhaven/swarmz-coding-agent";
...
- **Key detection functions removed from `@midhaven/swarmz-tui`**: All `isXxx()` key detection functions (`isEnter()`, `isEscape()`, `isCtrlC()`, etc.) have been removed. Use `matchesKey(data, keyId)` instead (e.g., `matchesKey(data, "enter")`, `matchesKey(data, "ctrl+c")`). This affects hooks and custom tools that use `ctx.ui.custom()` with keyboard input handling. ([#405](https://github.com/badlogic/pi-mono/pull/405))
...
- Configurable keybindings via `~/.swarmz/agent/keybindings.json`. All keyboard shortcuts (editor navigation, deletion, app actions like model cycling, etc.) can now be customized. Supports multiple bindings per action. ([#405](https://github.com/badlogic/pi-mono/pull/405) by [@hjanuschka](https://github.com/hjanuschka))
...
- Terminal window title now displays "swarmz - dirname" to identify which project session you're in ([#407](https://github.com/badlogic/pi-mono/pull/407) by [@kaofelix](https://github.com/kaofelix))
...
- `Attachment` type removed. Use `ImageContent` from `@midhaven/swarmz-ai` instead. Add images directly to message content arrays.
...
- `/share` command to upload session as a secret GitHub gist and get a shareable URL via swarmz.dev ([#380](https://github.com/badlogic/pi-mono/issues/380))
...
- **Sessions saved to wrong directory**: In v0.30.0, sessions were being saved to `~/.swarmz/agent/` instead of `~/.swarmz/agent/sessions/<encoded-cwd>/`, breaking `--resume` and `/resume`. Misplaced sessions are automatically migrated on startup. ([#320](https://github.com/badlogic/pi-mono/issues/320) by [@aliou](https://github.com/aliou))
...
- **SessionManager API**: The second parameter of `create()`, `continueRecent()`, and `list()` changed from `agentDir` to `sessionDir`. When provided, it specifies the session directory directly (no cwd encoding). When omitted, uses default (`~/.swarmz/agent/sessions/<encoded-cwd>/`). `open()` no longer takes `agentDir`. ([#313](https://github.com/badlogic/pi-mono/pull/313))

### Added

- **`--session-dir` flag**: Use a custom directory for sessions instead of the default `~/.swarmz/agent/sessions/<encoded-cwd>/`. Works with `-c` (continue) and `-r` (resume) flags. ([#313](https://github.com/badlogic/pi-mono/pull/313) by [@scutifer](https://github.com/scutifer))
...
- **Automatic custom system prompt loading**: Swarmz now auto-loads `SYSTEM.md` files to replace the default system prompt. Project-local `.swarmz/SYSTEM.md` takes precedence over global `~/.swarmz/agent/SYSTEM.md`. CLI `--system-prompt` flag overrides both. ([#309](https://github.com/badlogic/pi-mono/issues/309))
...
- **Credential storage refactored**: API keys and OAuth tokens are now stored in `~/.swarmz/agent/auth.json` instead of `oauth.json` and `settings.json`. Existing credentials are automatically migrated on first run. ([#296](https://github.com/badlogic/pi-mono/issues/296))
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@midhaven/swarmz-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **API keys in settings.json**: Store API keys in `~/.swarmz/agent/settings.json` under the `apiKeys` field (e.g., `{ "apiKeys": { "anthropic": "sk-..." } }`). Settings keys take priority over environment variables. ([#295](https://github.com/badlogic/pi-mono/issues/295))
...
- **Session hooks API redesign**: Merged `branch` event into `session` event. `BranchEvent`, `BranchEventResult` types and `swarmz.on("branch", ...)` removed. Use `swarmz.on("session", ...)` with `reason: "before_branch" | "branch"` instead. `AgentSession.branch()` returns `{ cancelled }` instead of `{ skipped }`. `AgentSession.reset()` and `switchSession()` now return `boolean` (false if cancelled by hook). RPC commands `reset`, `switch_session`, and `branch` now include `cancelled` in response data. ([#278](https://github.com/badlogic/pi-mono/issues/278))
...
- **Project-specific settings**: Settings now load from both `~/.swarmz/agent/settings.json` (global) and `<cwd>/.swarmz/settings.json` (project). Project settings override global with deep merge for nested objects. Project settings are read-only (for version control). ([#276](https://github.com/badlogic/pi-mono/pull/276))
...
- **Process suspension**: Press `Ctrl+Z` to suspend swarmz and return to the shell. Resume with `fg` as usual. ([#267](https://github.com/badlogic/pi-mono/pull/267) by [@aliou](https://github.com/aliou))
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@midhaven/swarmz-coding-agent` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **Global AGENTS.md loaded twice**: Fixed global AGENTS.md being loaded twice when present in both `~/.swarmz/agent/` and the current directory. ([#239](https://github.com/badlogic/pi-mono/pull/239) by [@aliou](https://github.com/aliou))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@midhaven/swarmz-coding-agent` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@midhaven/swarmz-ai`, `@midhaven/swarmz-tui`, `@midhaven/swarmz-coding-agent`).
...
- Fixed custom tools failing to load from `~/.swarmz/agent/tools/` when swarmz is installed globally. Module imports (`@sinclair/typebox`, `@midhaven/swarmz-tui`, `@midhaven/swarmz-ai`) are now resolved via aliases.
...
- Updated `@midhaven/swarmz-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@midhaven/swarmz-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@midhaven/swarmz-ai` with interleaved thinking support for Anthropic models.
...
- **Git branch indicator now works in subdirectories**: The footer's git branch detection now walks up the directory hierarchy to find the git root, so it works when running swarmz from a subdirectory of a repository. ([#156](https://github.com/badlogic/pi-mono/issues/156))
...
- **Collapse changelog setting**: Add `"collapseChangelog": true` to `~/.swarmz/agent/settings.json` to show a condensed "Updated to vX.Y.Z" message instead of the full changelog after updates. Use `/changelog` to view the full changelog. ([#148](https://github.com/badlogic/pi-mono/issues/148))
