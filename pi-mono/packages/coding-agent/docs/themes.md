> swarmz can create themes. Ask it to build one for your setup.
...
Swarmz loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.swarmz/agent/themes/*.json`
- Project: `.swarmz/themes/*.json`
- Packages: `themes/` directories or `swarmz.themes` entries in `package.json`
...
On first run, swarmz detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.swarmz/agent/themes
vim ~/.swarmz/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/badlogic/swarmz/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, swarmz reloads it automatically for immediate visual feedback.
...
Swarmz uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, swarmz falls back to the nearest approximation.
