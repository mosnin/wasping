...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want swarmz to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically to `~/.swarmz/agent/sessions/`, organized by working directory.

```bash
swarmz -c                  # Continue most recent session
swarmz -r                  # Browse and select a session
swarmz --no-session        # Ephemeral mode; do not save
swarmz --session <path|id> # Use a specific session file or session ID
swarmz --fork <path|id>    # Fork a session into a new session file
```
...
- `.swarmz/SYSTEM.md` for a project
- `~/.swarmz/agent/SYSTEM.md` globally
...
See [Swarmz Packages](packages.md) for package sources and security notes.
...
```bash
swarmz --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `PI_CODING_AGENT_DIR` | Override config directory; default is `~/.swarmz/agent` |
...
Swarmz keeps the core small and pushes workflow-specific behavior into extensions, skills, prompt templates, and packages.

It intentionally does not include built-in MCP, sub-agents, permission popups, plan mode, to-dos, or background bash. You can build or install those workflows as extensions or packages, or use external tools such as containers and tmux.

For the full rationale, read the [blog post](https://mariozechner.at/posts/2025-11-30-swarmz-coding-agent/).
