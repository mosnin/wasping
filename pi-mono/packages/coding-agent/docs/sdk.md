> swarmz can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to swarmz's agent capabilities. Use it to embed swarmz in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@midhaven/swarmz-coding-agent";
...
```bash
npm install @midhaven/swarmz-coding-agent
```
...
```typescript
import { createAgentSession } from "@midhaven/swarmz-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `swarmz.sendMessage()`.
...
The `Agent` class (from `@midhaven/swarmz-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.swarmz/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.swarmz/extensions/`)
- Project skills:
  - `.swarmz/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.swarmz/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.swarmz/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@midhaven/swarmz-ai";
import { AuthStorage, ModelRegistry } from "@midhaven/swarmz-coding-agent";
...
// Default: uses ~/.swarmz/agent/auth.json and ~/.swarmz/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@midhaven/swarmz-coding-agent";
...
- If you omit `tools`, swarmz automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@midhaven/swarmz-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `swarmz.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `swarmz.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.swarmz/agent/extensions/`, `.swarmz/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `swarmz.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@midhaven/swarmz-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@midhaven/swarmz-coding-agent";
...
1. Global: `~/.swarmz/agent/settings.json`
2. Project: `<cwd>/.swarmz/settings.json`
