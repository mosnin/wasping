...
Swarmz saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.swarmz/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
swarmz -c                  # Continue most recent session
swarmz -r                  # Browse and select from past sessions
swarmz --no-session        # Ephemeral mode; do not save
swarmz --session <path|id> # Use a specific session file or partial session ID
swarmz --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `swarmz -r` opens the same picker at startup.
...
When available, swarmz uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `swarmz -r`.
...
When `/tree` switches away from one branch to another, swarmz can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
