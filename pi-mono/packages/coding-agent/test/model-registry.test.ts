...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@mariozechner/swarmz-ai";
import { getApiProvider } from "@mariozechner/swarmz-ai";
import { getOAuthProvider } from "@mariozechner/swarmz-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `swarmz-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
