...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "swarmz-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".swarmz", "prompts");
			...
			const projectSkillDir = join(cwd, ".swarmz", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".swarmz", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".swarmz", "themes"), { recursive: true });
		...
		it("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(swarmz) {
	swarmz.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".swarmz"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".swarmz", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".swarmz", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".swarmz", "extensions");
	...
	swarmz.registerCommand("project-only", {
	...
	swarmz.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.swarmz", async () => {
			const piDir = join(cwd, ".swarmz");
