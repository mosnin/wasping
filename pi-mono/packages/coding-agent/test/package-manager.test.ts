...
Content`,
		...
		it("should auto-discover root markdown skills from .swarmz skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .swarmz", async () => {
			const extDir = join(tempDir, ".swarmz", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".swarmz"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".swarmz", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".swarmz", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".swarmz", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".swarmz", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".swarmz", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json swarmz.extensions in extensions setting", async () => {
			// Create a package with swarmz.extensions in package.json
			...
			// Should find the extensions declared in package.json swarmz.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".swarmz", "agent");
		...
		it("should dedupe user skill entries when ~/.swarmz/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .swarmz auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".swarmz\n");

			const skillDir = join(tempDir, ".swarmz", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with swarmz manifest", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".swarmz", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .swarmz settings base", () => {
			...
			const rel = relative(join(tempDir, ".swarmz"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in swarmz manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json swarmz.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".swarmz", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".swarmz", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".swarmz", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".swarmz", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".swarmz", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".swarmz", "npm", "node_modules", "example");
