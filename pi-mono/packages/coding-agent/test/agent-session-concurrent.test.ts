...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `swarmz-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(swarmz) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = swarmz;
			},
			(swarmz) => {
		...
		expect(swarmz).toBeDefined();

		swarmz!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
