...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@mariozechner/swarmz-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.swarmz/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".swarmz", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.swarmz/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.swarmz/agent/auth.json
 */
...
/** Path to the real swarmz agent config directory */
export const PI_AGENT_DIR = join(homedir(), ".swarmz", "agent");

/**
 * Get an AuthStorage instance backed by ~/.swarmz/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `swarmz-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
