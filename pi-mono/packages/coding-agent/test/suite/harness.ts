...
import type { AgentMessage, AgentTool } from "@mariozechner/swarmz-agent-core";
import { Agent } from "@mariozechner/swarmz-agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@mariozechner/swarmz-ai";
import { registerFauxProvider } from "@mariozechner/swarmz-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `swarmz-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
