...
import { Agent, type AgentEvent, type AgentTool } from "@mariozechner/swarmz-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@mariozechner/swarmz-ai";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `swarmz-retry-test-${Date.now()}`);
