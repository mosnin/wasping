...
import type { AgentTool } from "@midhaven/swarmz-agent-core";
...
import { createAssistantMessageEventStream } from "@midhaven/swarmz-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `swarmz-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
